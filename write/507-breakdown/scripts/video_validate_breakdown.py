...
from video_describe_key_frames import valid_anchor_checks
...
def main()->int:
 ...
 for u in units:
  ...
  for w in u.get("candidateWindows",[]):
   if w["evidence"]=="image_visual_anchor" and not any(valid_anchor_checks(o.get("anchorChecks"),visual) and o.get("semanticUnit")==u.get("id") and (o.get("semanticUnit"),o.get("pts"),o.get("frame")) in allowed and (ws/o.get("frame","")).is_file() and o.get("description")==w.get("text") and w["start"]<=o.get("pts",-999)<=w["end"] for o in observations): fail("图片窗口缺少观察原件")
