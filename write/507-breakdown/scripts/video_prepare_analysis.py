...
def main()->int:
 ...
 valid_ids={u.get("id") for u in locations.get("semanticUnits",[])}
 allowed={(w.get("semanticUnit"),f["pts"],f["frame"]) for w in adaptive.get("windows",[]) for f in w.get("frames",[]) if w.get("semanticUnit") in valid_ids}
 ...
 for item in observations["observations"]:
  ...
  if not isinstance(item.get("pts"),(int,float)) or not math.isfinite(item["pts"]) or item.get("semanticUnit") not in valid_ids or frame.is_absolute() or ".." in frame.parts or not str(frame) or not isinstance(item.get("description"),str) or not item["description"].strip() or not (ws/frame).is_file() or not (ws/frame).resolve().is_relative_to(ws.resolve()) or (item.get("semanticUnit"),item["pts"],item["frame"]) not in allowed:
