...
7. **资源分发策略**：对提示词、模板、清单、附件等可领取资源，必须明确选择“图卡全文交付 / 评论区钩子 / 拆为系列 / 外链或附件领取”之一；写清完整资源仍存在哪里，以及图卡怎样准确承诺。该选择涉及用户承诺时进入 `507-grill` 确认；评论区钩子未经确认不能使用，也不能伪装成正文已经交付。
...
- 默认先完成 `copy-spec.md`。若其中仍有删除承重内容、资源分发、中心判断或发布承诺等用户取舍，携带规格进入 `507-grill` 后返回；仅剩分页、样式和工程实现时由 agent 自行设计并告知用户。用户明确要求直接生成时可不等待逐项审阅，但仍必须落规格并通过同一验收。
