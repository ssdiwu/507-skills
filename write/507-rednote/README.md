...
它先锁定中心判断与论证链，再按主稿章节生成 `copy-spec.md` 和结构化 `rednote-project.json`；封面和正文逻辑块用 `sourceMap` 逐项承接保真清单编号。长文由浏览器按真实排版高度从页头自动填到页脚，放不下的段落才移到新页，最后导出 1500×2000 JPG、联系表和渲染清单。只有渲染清单的规格 SHA-256 与当前项目规格一致时，产物才可交付；续页默认不重复标题。
