...
#### `507-mine` 挖矿
写作流的最上游。吃三种素材——用户自己的想法、外部文章、视频拉片内容——先看 vault 既有碎片做有目的的联网搜索，收正反例让观点立体，产出多元碎片边产边落进 `01-碎片/`。
- **不产选题**（归 fuse），**不写正文**（归 forge）
- 默认外搜 + 收正反例（类似 3-5 篇 + 反例 1-5 篇），除非用户指定了具体素材
...
#### `507-fuse` 熔炼
碎片融合成候选 idea。检索 `01-碎片/` 既有碎片 + 本次新碎片，挑时效性/主体性碎片作骨架，产出候选 idea 到 `02-创意/候选/`，由用户审核融合质量。
...
#### `507-forge` 锻造
书面成文与共创收敛。读取创意区的中心判断、论证链和证据边界，先写出完整版本，再吸收用户的判断统一重写，直到形成可发布主稿。
- 没有成熟创意或证据不足时，Agent 自动转入 mine / fuse，不让用户手动调度技能链
