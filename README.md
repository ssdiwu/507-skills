...
本仓库按 [Agent Skills 规范](https://agentskills.io/specification)组织每个 `SKILL.md`（技能说明），不绑定某个 Agent（智能体）产品。共享 skill 定义动作、边界、产物、验收和出口合同；技能发现、工具权限、显式调用与调度方式由目标宿主决定，不属于本仓库的通用承诺。
...
flowchart TD
    ...
    delivery --> code["code/<br/>产品规格 · 工单 · 实施 · 审查 · 提交 · 发布"]
...
explore / research / prototype / grill（均按需）
    ...
    fix / test / tdd / simplify / 正常实现
                    ↓
              review → commit（明确要求时）→ release（明确要求发布时）
```
...
| `507-release` | 明确要求发布时，定版本、打 tag、推送并按探测渠道发布 |
