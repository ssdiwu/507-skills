...
用户的代码工作流，按**建造隐喻**命名——把代码工程类比为建造工序：奠基整地、画蓝图、做样板、施工、验收巡检。
...
#### `507-ground` 奠基
为一个或多个代码项目做项目工作流初始化/巡检。检查项目自己的 AGENTS.md、README.md、CHANGELOG.md、doc/、doc/术语表.md、doc/决策档案/、分层 README 和已口述但未沉淀的约束。
- 不在执行链里，是项目环境维度的入口，可独立运行
- 触发："setup用户""项目初始化""巡检这些项目"
...
#### `507-workorder` 开工单
把任务结构化成可上传 GitHub、可被 AI 领取执行的 **issue 工单**。正向把方案/PRD 拆成可领取 issue，反向把 bug/gap 落成可追踪 issue。模板以经实践验证的 issue 模式为骨架（正向八板块 + 反向现象/影响/方案/验收/风险）。
