---
name: 507-handover
description: "会话移交：中断、换会话、压缩上下文或转交给另一 Agent 时，在对话中生成脱敏的临时交接摘要，不写文件。Use when user says handoff, handover, 交接, 换会话, 转交, 压缩上下文, 给下一个 agent 摘要."
---
...
- 不泄露敏感信息；
- 不把建议写成已经完成；
- 不把会话交接当作默认收尾，只有确实中断、转交或压缩上下文时使用。
