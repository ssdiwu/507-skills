...
4. **先过决策所有权闸门，再问用户**：能通过代码、文档、既有共识、工程常识或小型验证自行决定的，不要提问。遇到需要环境事实的问题，立即用工具或子代理核实；把该事实视为未决前置条件，仅冻结依赖它的问题，其余 frontier 不等待、照常推进。不得把可查事实转成用户问题。API schema（模式）、参数命名、单次上限、模块边界、兼容策略、状态细节、测试与工程验证默认由 agent 收口；只有会改变产品意图、权责边界、不可逆成本、体验取舍或风险承受的选择，才交给用户。若只有一个合理结论，直接说明判断并执行，不伪造候选。
...
5. 在范围内按 frontier 追问，一个分支一个分支走：每轮仅收集前置条件已确定的独立决策；有合适工具时批量收集，否则一次只问一个问题；每轮回答后重新计算 frontier；每走完一个顶层分支，输出一块累积共识。
6. 每敲定一个术语 / 决策，当场从共识块里"提拔"到对应 doc 文件。
7. 所有顶层分支走完，agent 自行补全 API、实现切片和验证方案；给用户一句话总结对齐结果。仅当用户未在原始请求中明确授权实施时，再请其确认共同理解后进入实现或交接。
