...
在真正动手改代码之前，用一轮结构化追问把计划问透：先与用户框定要解决的顶层分支，再在范围内沿依赖关系逐个问到底，每走完一个分支收束固化一次，走完即结束。对齐过程中确定的术语当场沉淀进 `doc/术语表.md`（术语表），够格的决策记入 `doc/决策档案/`（架构决策记录）。
...
1. **一次只问一个问题**，等用户回答后再问下一个，绝不一次抛出一串。这里的“一个问题”可以有候选项，但不能把相互依赖的后续决策提前混进来。
...
> 只在通过“决策所有权闸门”后使用本模板。不要把 API（接口）字段、内部实现、测试路径、验证动作或已被排除的选项写进候选。
...
5. 在范围内逐个追问，一个分支一个分支走；每走完一个顶层分支，输出一块累积共识。
